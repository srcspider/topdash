...
/**
 * Given raw statement, checks if the statement is valid as UPDATE-only
 * statement.
 *
 * ## DISCLAIMER
 *
 ...
 */
export function applyDefaultSQLUpdateSanityCheck(
  rawStmt: string,
): string[] | nil {
  ...
  let match = rawStmt.match(
    /(CREATE|PROCEDURE|DELIMITER|CALL|DELETE|DO|HANDLER|IMPORT|INSERT|INTERSECT|LOAD|REPLACE|ALTER|DROP|INTO|RENAME)/,
  )

  if (match != nil) {
    let forbiddenKey = match[1]
    return [
      `Usage of forbidden keyword within UPDATE-only query (was: ${forbiddenKey}).`,
    ]
  }

  if (!/UPDATE/.test(rawStmt)) {
    return ["Not a recognizable UPDATE query."]
  }
